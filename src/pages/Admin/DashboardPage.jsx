...
import {
  getApplicationsByCampaignType,
  getApplicationsByMonth,
  getCampaignsByAirlinePartner,
  getCampaignsByMonth,
  getTestsByTestType,
  getUserHistoryByMonth,
} from "../../service/api";
...
const DashboardPage = () => {
  ...
  const [, setCampaignTypeData] = useState([]);
  const [, setTestTypeData] = useState([]);
  const [, setCampaignsByMonthData] = useState([]);
  const [, setApplicationsByMonthData] = useState([]);
  const [, setUserHistoryByMonthData] = useState([]);
  ...
  const updateCampaignsByMonth = async (ChartParam, yearParam) => {
    ...
    if (campaignsMonthChartRef.current && ChartInstance) {
      ...
      const chart = new ChartInstance(campaignsMonthChartRef.current, {
        ...
        data: {
          labels: apiData.map(
            (item) => monthNames[item.month - 1] || `M${item.month}`
          ),
          datasets: [
            {
              label: "Campaigns",
  ...
  useEffect(() => {
    ...
    const setupCharts = async () => {
      try {
        ...
        if (isCancelled) return;
        const airlineDataset = airlineApiData.length
          ? airlineApiData
          : airlineData;
        ...
        if (isCancelled) return;
        const campaignTypeDataset = campaignTypeApiData.length
          ? campaignTypeApiData
          : [];
    ...
    return () => {
      ...
      if (campaignTypeChartInstance.current)
        campaignTypeChartInstance.current.destroy();
  ...
  const handleCampaignsYearUpdate = async (yearParam) => {
    ...
    await updateCampaignsByMonth(
      ChartInstanceRef.current,
      yearParam ?? campaignsYear
    );
  };
  ...
  const handleApplicationsYearUpdate = async (yearParam) => {
    ...
    await updateApplicationsByMonth(
      ChartInstanceRef.current,
      yearParam ?? applicationsYear
    );
  };
  ...
  const handleUserHistoryYearUpdate = async (yearParam) => {
    ...
    await updateUserHistoryByMonth(
      ChartInstanceRef.current,
      yearParam ?? userHistoryYear
    );
  };
  ...
  return (
    <div className="w-full h-full p-5">
      ...
      <div className="flex flex-wrap gap-5 mb-8">
        ...
        <div className="bg-white rounded-lg p-6 shadow-md flex-1 min-w-[300px]">
          <h2 className="text-gray-800 text-xl font-semibold mb-5">
            Applications by Campaign Type
          </h2>
          <div className="w-full flex justify-center">
            <canvas
              ref={campaignTypeChartRef}
      ...
      <div className="flex flex-wrap gap-5 mb-8">
        <div className="bg-white rounded-lg p-6 shadow-md flex-1 w-full">
          <div className="flex justify-between items-center mb-5 flex-wrap gap-3">
            <h2 className="text-gray-800 text-xl font-semibold">
              User History by Month (Pass/Failed)
            </h2>
            <div className="flex items-center gap-3">
              <label
                htmlFor="userHistoryYear"
                className="font-semibold text-gray-600"
              >
                Year:
              </label>
