...
import { createTestType } from "../../../service/api2";

const CreateTestTypeModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await createTestType(trimmedName);

            if (result.success) {
                toast.success(result.message || "Create test type successfully");
                if (onSuccess) {
                    onSuccess();
                }
                onClose();
            } else {
                toast.error(result.error || "Cannot create test type");
            }
        } catch (err) {
            console.error("Error creating test type:", err);
            toast.error("An error occurred while creating test type");
        } finally {
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            Enter the name of the new test type.
                        </p>
                    </div>
                    <button
                        onClick={onClose}
