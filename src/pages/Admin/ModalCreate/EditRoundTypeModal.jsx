...
import { updateRoundType } from "../../../service/api";

const EditRoundTypeModal = ({ isOpen, onClose, roundType, onSuccess }) => {
    ...
    useEffect(() => {
        if (isOpen && roundType) {
            setRoundTypeName(roundType.name || roundType.roundTypeName || "");
            setError("");
        }
    }, [isOpen, roundType]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Gửi đúng format body là chuỗi đơn theo Swagger: "Updated round"
            const result = await updateRoundType(roundType.id, trimmedName);

            if (result.success) {
                toast.success(result.message || "Update round type successfully");
                if (onSuccess) {
                    onSuccess();
                }
                onClose();
            } else {
                toast.error(result.error || "Cannot update round type");
            }
        } catch (err) {
            console.error("Error updating round type:", err);
            toast.error("An error occurred while updating round type");
        } finally {
...
export default EditRoundTypeModal;
