          ...
          if (res.success && Array.isArray(res.data)) {
            let mapped = res.data
              .map((item, index) => {
                ...
                const itemCampaignType = item?.campaignType || "";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <section className="p-6 bg-white border shadow-sm rounded-2xl border-slate-200">
                ...
                <th colSpan={7} className="px-4 py-3">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap gap-3">
                      {roundTypes.map((rt) => {
                        const isSelected =
                          selectedRoundType === rt.roundTypeId ||
                ...
                items.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-700">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${campaignTypeBadge}`}
                        >
                          {campaignTypeLabel}
                        </span>
                      </td>
        ...
        {/* Pagination */}
        {!isLoading && items.length > 0 && pagination.totalRecords > 0 && (
