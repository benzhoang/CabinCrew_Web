    ...
    () => async () => {
      ...
      try {
        ...
        if (res.success && Array.isArray(res.data)) {
          ...
          setError(res.error || "Cannot load airline partner list");
          setItems([]);
          setCurrentPage(1);
        }
      } catch (err) {
        setError(err.message || "Cannot load airline partner list");
        setItems([]);
        setCurrentPage(1);
      } finally {
        setIsLoading(false);
    ...
    <div className="p-6 space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">List of airline partners</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading ? (
                ...
                displayedItems.map((item, index) => {
                  ...
                  const partnerId = item?.partnerId ?? item?.id ?? index + 1;
                  const partnerName = item?.partnerName || item?.name || "N/A";
