...
const convertRoundsToBatches = (rounds) => {
  ...
  return rounds.map((round, index) => {
    ...
    const mappedStatus =
      statusMap[round.status] || round.status?.toLowerCase() || "completed";
...
const BatchManagement = ({ campaign }) => {
  const [currentBatches, setCurrentBatches] = useState(() => {
    ...
    if (
      ...
      console.log(
        "BatchManagement: Found rounds in campaign:",
        campaign.rounds
      );
  ...
  useEffect(() => {
    ...
    if (campaign?.rounds && Array.isArray(campaign.rounds)) {
      console.log(
        "BatchManagement: Updating batches from rounds:",
        campaign.rounds
      );
      ...
      console.log(
        "BatchManagement: Updating batches from batches array:",
        campaign.batches
      );
  ...
  // Lấy campaignType từ campaign object (tương tự như trong DetailInfo)
