...
const ExamTask = () => {
  ...
  const mapStatusValue = (status) => {
    ...
    if (["inprogress", "processing", "ongoing"].includes(compact))
      return "inProgress";
    if (["cancelled", "canceled", "cancel", "aborted"].includes(compact))
      return "cancelled";
  ...
  const transformTasksData = (data) => {
    ...
    if (Array.isArray(data)) {
      data.forEach((campaign) => {
        ...
        const campaignName =
          campaign.campaignName || campaign.name || "Unnamed campaign";
        ...
        if (Array.isArray(campaign.assignments)) {
          campaign.assignments.forEach((assignment) => {
            allTasks.push({
              ...
              taskDescription:
                assignment.taskDescription || assignment.description || "",
  ...
  useEffect(() => {
    ...
    if (taskFilter !== "all") {
      filtered = filtered.filter(
        (task) => normalizeString(task.task) === normalizeString(taskFilter)
      );
    }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-slate-800">
                        {formatDateValue(task.assignedAt)}
                      </p>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-slate-800">
                      {formatDateValue(selectedTask.assignedAt)}
                    </p>
                  </div>
                  {selectedTask.completedAt && (
                    <div>
                      ...
                      <p className="font-medium text-green-600">
                        {formatDateValue(selectedTask.completedAt)}
                      </p>
                    </div>
                  )}
                  <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-slate-800">
                    {formatDateOnly(selectedTask.startDate)} -{" "}
                    {formatDateOnly(selectedTask.endDate)}
                  </p>
                </div>
