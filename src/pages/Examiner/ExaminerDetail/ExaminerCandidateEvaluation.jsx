...
const mapCriteriaToEvaluations = (criteriaGroups, previousEvaluations = {}) => {
  ...
  criteriaGroups?.forEach((group) => {
    group?.items?.forEach((item) => {
      if (!item) return;
      const hasId =
        typeof item.interviewCriteriaItemId !== "undefined" &&
        item.interviewCriteriaItemId !== null;
      ...
      mapped[key] = {
        score:
          typeof previousValue.score === "number" ? previousValue.score : 10,
...
const ExaminerCandidateEvaluation = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (submittedCount >= 3) {
      toast.error(
        "You have already submitted 3 times. Cannot submit more evaluations."
      );
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          response.error || "Unable to submit evaluation. Please try again."
        );
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white/90">
                  Evaluate interview criteria for the applicant
                </p>
              </div>
            </div>
      ...
      <div className="px-6 py-8 mx-auto max-w-7xl">
        ...
        <div className="mb-6 space-y-6">
          {criteriaLoading ? (
            <div className="flex items-center justify-between p-6 bg-white border shadow-sm rounded-xl border-slate-200">
              <div>
                <h2 className="text-lg font-semibold text-slate-800">
                  Loading interview criteria...
                </h2>
            ...
            <div className="p-6 bg-white border border-red-200 shadow-sm rounded-xl">
              <p className="font-medium text-red-600">{criteriaError}</p>
              <button
                onClick={loadInterviewCriterias}
                className="inline-flex items-center gap-2 px-4 py-2 mt-4 text-white transition-colors bg-blue-600 rounded-md hover:bg-blue-700"
              >
                Retry
              </button>
            </div>
                    ...
                    <tbody>
                      {Array.isArray(group.items) && group.items.length > 0 ? (
                        group.items.map((item, itemIndex) => {
                          const hasId =
                            typeof item?.interviewCriteriaItemId !==
                              "undefined" &&
                            item?.interviewCriteriaItemId !== null;
                          const criterionKey = hasId
                            ? item.interviewCriteriaItemId
                            : item?.criteria ?? `${groupIndex}-${itemIndex}`;
                          const evaluation = evaluations[criterionKey] || {
                            score: 10,
                            comment: "",
                          };
                              ...
                              <td className="px-4 py-3 text-center align-middle">
                                <div className="flex justify-center">
                                  <select
                                    ...
                                    className="w-20 px-2 py-1 text-center border rounded-md border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  >
                        ...
                        <tr>
                          <td
                            colSpan="3"
                            className="py-4 text-sm text-center text-slate-500"
                          >
