...
const ExaminerAppearancePage = () => {
  ...
  const fetchResult = useCallback(async () => {
    ...
    try {
      ...
      if (resultResponse.success) {
        ...
        setError(
          resultResponse.error || "Unable to load appearance check result."
        );
        setResult(null);
      }
    } catch (err) {
  ...
  const getCriteriaInfo = useCallback(
    (criteria) => {
      const itemId =
        criteria.scoringCriteriaItemId ||
        criteria.criteriaId ||
        criteria.id ||
                              ...
                              <tbody className="bg-white divide-y divide-gray-200">
                                {criteriaGroup.map((criteria) => {
                                  ...
                                  const criteriaName =
                                    ...
                                    `Tiêu chí ${criteria.originalIndex + 1}`;
                                  const isPassed =
                                    criteria.isPassed ??
                                    criteria.result ??
                                    criteria.score;
                                  return (
                                    ...
                                    >
                                      <td className="px-4 py-3 text-sm">
                                        ...
                                        {Array.isArray(criteriaInfo.details) &&
                                          criteriaInfo.details.length > 0 && (
                                            <ul className="mt-2 space-y-1">
                                              {criteriaInfo.details.map(
                                                (detail, detailIndex) => (
                                                  <li
                                                    key={
                                                      detail.detailText ||
                                      ...
                                      <td
                                        className={`px-4 py-3 text-sm align-top ${getResultColorClass(
                                          isPassed
                                        )}`}
                                      >
...
export default ExaminerAppearancePage;
