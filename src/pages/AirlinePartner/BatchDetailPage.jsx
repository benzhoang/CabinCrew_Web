...
import { getCampaignRoundById, getRoundParticipants, exportFlightHoursConfirmation, importFlightHoursConfirmation } from '../../service/api'
...
const BatchDetailPage = () => {
  ...
  const isFlightHoursConfirmationRound = useMemo(() => {
    ...
    const selectedRound = availableRounds.find(r => String(r.roundId) === String(roundFilter))
    if (selectedRound) {
      const roundName = selectedRound.roundName?.toLowerCase() || ''
      return roundName.includes('flight hours') || roundName.includes('flight hours confirmation')
    }
    return false
  ...
  const currentCampaignRoundId = useMemo(() => {
    return params.id || batchData?.batch?.id || batchData?.batch?.campaignRoundId || batchData?.campaignRoundId
  }, [params.id, batchData])
  ...
  const handleExportFlightHours = async (roundId, campaignRoundId) => {
    ...
    console.log('handleExportFlightHours - roundId:', roundId, 'campaignRoundId:', campaignRoundId)
  ...
  const handleImportFlightHours = async (file, roundId, campaignRoundId) => {
    if (!roundId) {
      return {
        success: false,
        error: 'Không tìm thấy thông tin vòng để import'
      }
    }
    ...
    if (!file) {
      return {
        success: false,
        error: 'Bạn chưa chọn file Excel để tải lên'
      }
    }

    try {
      console.log('Calling importFlightHoursConfirmation with roundId:', roundId)
      const result = await importFlightHoursConfirmation(roundId, file)
      console.log('Import result:', result)

      if (result.success) {
        // Hiển thị toast thành công
        const message = result.message || `Import thành công! Đã xử lý ${result.totalProcessed || 0} ứng viên (${result.passedCount || 0} đạt, ${result.failedCount || 0} không đạt).`
        ...
        if (campaignRoundIdToUse) {
          setTimeout(() => {
            navigate(`/airline-partner/campaigns/${campaignRoundIdToUse}/candidate`)
        ...
        return {
          success: false,
          error: result.error || 'Import file thất bại'
        }
      }
    } catch (error) {
