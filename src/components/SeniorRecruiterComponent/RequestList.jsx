...
const RequestList = ({
  ...
  const [statusFilter] = useState("approved"); // Default to approved for Senior, no UI to change it
  ...
  // Map status filter to API status number
  const getStatusNumber = (statusFilter) => {
    if (statusFilter === "all") {
      return undefined; // Don't filter by status when "all" is selected
    }
    switch (statusFilter) {
      case "approved":
        return 2;
  ...
  const fetchRequests = async (page = 1, showLoading = true) => {
    ...
    try {
      ...
      if (statusNumber !== undefined) {
        baseParams.status = statusNumber; // Only add status if not "all"
      }
      if (partnerId) {
      ...
      // Helper function to normalize status from API (reuse logic from StatusBadge)
      ...
      const mappedRequests = items.map((item) => ({
        ...
        status: normalizeStatusFromAPI(item.status), // Normalize status from API
  ...
  // Helper function to normalize status (reuse logic from StatusBadge)
              ...
              <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-1 gap-3 mb-2 md:grid-cols-2 lg:grid-cols-6">
                    ...
                    <div>
                      ...
                      <div className="mt-1">
                        ...
                        >
                          {getRequestTypeLabel(campaign.requestType) ||
                            "No request type"}
                        </span>
                      </div>
                    </div>
                    <div>
                      ...
                      <p className="font-medium text-slate-800">
                        {(campaign.targetQuantity || 0) + " applicants"}
                      </p>
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-2 ml-4">
                  <button
                    onClick={() =>
                      navigate(
                        `/senior-recruiter/requests/${campaign.requestId}`
                      )
                    }
