...
const demoCampaigns = [
  ...
  },
  {
    ...
    description: 'Tuyển dụng kỹ thuật viên bảo trì máy bay, đang chờ phê duyệt từ ban quản lý.',
  ...
  },
  {
    ...
    description: 'Nâng bậc cho các cabin crew có kinh nghiệm lâu năm và thành tích xuất sắc.',
  ...
  },
  {
    ...
    description: 'Nâng bậc cho các IT Specialist xuất sắc lên vị trí Lead.',
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1 text-sm text-gray-700">
            ...
            <div>
          <span className="text-gray-500">Trạng thái:</span> <StatusBadge status={campaign.status} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button className="px-3 py-1.5 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700" onClick={() => navigate(`/senior-recruiter/campaigns/${campaign.id}`, { state: { campaign: campaign } })}>Xem chi tiết</button>
...
const CampaignList = ({ search = '', campaignTypeFilter = 'all', campaigns = demoCampaigns }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return campaigns.filter((c) => {
      ...
      const matchCampaignType = campaignTypeFilter === 'all' || c.campaignType === campaignTypeFilter
