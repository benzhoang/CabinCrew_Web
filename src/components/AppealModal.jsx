...
const AppealModal = ({ isOpen, onClose, onConfirm, testSessionId }) => {
  ...
  const handleConfirm = async () => {
    ...
    if (!appealReason.trim()) {
      setError(
        t("appeal_reason_required") || "Vui lòng nhập lý do yêu cầu phúc khảo"
      );
      return;
    }
    ...
    if (appealReason.trim().length < 10) {
      setError(
        t("appeal_reason_min_length") ||
          "Lý do phúc khảo phải có ít nhất 10 ký tự"
      );
      return;
    }
    ...
    if (appealReason.trim().length > 250) {
      setError(
        t("appeal_reason_max_length") ||
          "Lý do phúc khảo không được vượt quá 250 ký tự"
      );
      return;
    }
    ...
    if (!testSessionId) {
      setError("Not found test session. Please try again.");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        // Lỗi từ API
        setError(
          result.error || "Cannot send appeal request. Please try again."
        );
        setIsLoading(false);
      }
    } catch (err) {
        ...
        <div className="flex items-start">
          ...
          <div className="flex-1">
            ...
            <div className="mb-4">
              ...
              >
                {t("appeal_reason_label") || "Lý do yêu cầu phúc khảo"}{" "}
