...
const CabincrewActionForm = ({
  ...
  const handleSaveClick = useCallback(
    async (e) => {
      ...
      if (!applicationId) {
        toast.error(
          "Cannot find application form. Please reload the page and try again."
        );
        return;
      }
      ...
      try {
        ...
        const payload = {
          totalFlightHours: formData.totalFlightHours
            ? parseInt(formData.totalFlightHours, 10)
            : 0,
        ...
        if (result.success) {
          ...
          toast.error("Cannot update application. Please try again.");
        }
      } catch (error) {
