...
const AddressInformation = ({
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (normalized.length) {
            ...
            console.log(
              "[AddressInformation] Cities set successfully:",
              normalized
            );
          } else {
            console.warn(
              "[AddressInformation] City list from API is empty or invalid."
            );
            toast.error("Cannot load city list. Please try again later.");
          }
        } else {
          console.warn(
            "[AddressInformation] Cannot get city list:",
            result.error
          );
          toast.error("Cannot load city list. Please try again later.");
        }
      } catch (error) {
        console.error("[AddressInformation] Error loading city list:", error);
        toast.error("Cannot load city list. Please try again later.");
      } finally {
  ...
  useEffect(() => {
    const fetchWards = async () => {
      console.log(
        "[AddressInformation] Starting to fetch wards, cityId:",
        formData.cityId
      );
      if (!formData.cityId) {
        setWards([]);
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (normalized.length) {
            ...
            uniqueWardNames.sort((a, b) =>
              a.localeCompare(b, "vi", { sensitivity: "base" })
            );
            setWards(uniqueWardNames);
            console.log(
              "[AddressInformation] Wards set successfully:",
              uniqueWardNames
            );
            console.log(
              "[AddressInformation] Wards data with IDs:",
              normalized
            );
          } else {
            console.warn(
              "[AddressInformation] Ward list from API is empty or invalid."
            );
            toast.error("Cannot load ward list. Please try again later.");
          }
        } else {
          console.warn(
            "[AddressInformation] Cannot get ward list:",
            result.error
          );
          toast.error("Cannot load ward list. Please try again later.");
        }
      } catch (error) {
        console.error("[AddressInformation] Error loading ward list:", error);
        toast.error("Cannot load ward list. Please try again later.");
      } finally {
  ...
  useEffect(() => {
    console.log(
      "[AddressInformation] Checking city sync, formData.city:",
      formData.city,
      "cities:",
      cities
    );
    if (formData.city && cities.length > 0) {
      ...
      if (selectedCity && formData.cityId !== selectedCity.id) {
        console.log(
          "[AddressInformation] Updating cityId to:",
          selectedCity.id
        );
        setFormData((prev) => ({
          ...prev,
        ...
        console.log(
          "[AddressInformation] Adding new city to list, cityId:",
          formData.cityId
        );
        setCities((prev) => [
          ...prev,
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            ...
            <SearchableDropdown
              ...
              onChange={(e) => {
                console.log(
                  "[AddressInformation] City changed to:",
                  e.target.value
                );
              ...
              placeholder={isCitiesLoading ? "Loading..." : t("select_city")}
              error={!!errors.city}
              label="city/province"
          ...
          <div>
            ...
            <SearchableDropdown
              ...
              onChange={(e) => {
                console.log(
                  "[AddressInformation] Ward changed to:",
                  e.target.value
                );
              ...
              placeholder={isWardsLoading ? "Loading..." : t("select_ward")}
